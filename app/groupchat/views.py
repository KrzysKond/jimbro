...
class GroupMessagesView(APIView):
    ...
    def get(self, request, group_id):
        ...
        if not group.members.filter(id=request.user.id).exists():
            return Response({"status": False, "message": "You are not a member of this group."}, status=status.HTTP_403_FORBIDDEN)
...
class GroupListView(APIView):
    ...
    def get(self, request):
        """Retrieve all groups for the authenticated user, ensuring they are members."""
        user = request.user
